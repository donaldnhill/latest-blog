...
class td_woo_attribute_filter extends td_block {
	...
	function render($atts, $content = null) {
        ...
        $tax_status = []; // Array to store the status of each taxonomy
        if ( !empty( $taxonomies ) && is_array( $taxonomies ) ) {
            foreach ($taxonomies as $taxonomy) {
                // Get the taxonomy name
                $tax_name = $taxonomy->attribute_name ?? ($taxonomy->name ?? '');

                // added a new option for single/multi selection, the old multi option is named OR/AND
                // $tax_name . '_single_selection is single selection when it is unchecked
                $tax_status[$tax_name . '_is_single'] = $this->get_att($tax_name . '_single_selection');
            }
        }
                ...
                $buffy .= '<div class="tdw-filters-wrap">';
                    ...
                    foreach ( $filters_data['taxonomies'] as $taxonomy ) {
                        ...
                        foreach ( $terms as $term ) {
                            ...
                            $is_single_selection = !empty($tax_status[$tax_name . '_is_single']);

                            if ( $hide && $is_single_selection ) {
                                continue;
                            }
	                    ...
	                    //$buffy .= '<div style="color: orangered; font-weight: bold;">' . $type . '</div>';
                            ...
                            switch ( $type ) {
                                ...
                                default:
                                    $single_selection = '';
                                    if (strpos($tax, 'pa_') === 0) {
                                        $attr_tax = str_replace('pa_', '', $tax) . '_is_single';
                                        $single_selection = $tax_status[$attr_tax];
                                    }

                                $buffy .= sprintf(
                                    '<ul class="tdw-filter-items-wrapper %1$s" data-is-single="%2$s" data-taxonomy="tdw_%3$s">%4$s</ul>',
                                    trim(implode(' ', array("{$type}-list-wrapper"))),
                                    $single_selection, // This will be 'yes' or '' (empty string)
                                    $tax,
                                    $data
                                );
                                break;
